            ...
            if (up_len != 0.0f) {
                ...
                for (; primitive_count != 0; primitive_count--) {
                    ...
                    for (i = count; i > 0; i--) {
                        ...
                        GX_FIFO_F32(up.x * tx + (right.x * sx + x));
                        GX_FIFO_F32(up.y * tx + (right.y * sx + y));
                        GX_FIFO_F32(up_z * tx + (right_z * sx + z));
...
static inline void psDispSubAppSRT(HSD_Particle* pp, u8* texform)
{
    ...
    if (pp->kind & Trail) {
        ...
        GX_FIFO_F32(-ax + cur_x);
        GX_FIFO_F32(-ay + cur_y);
        GX_FIFO_F32(cur_z);
        if (pp->kind & DispTexture) {
            GX_FIFO_U8((pp->kind >> 16) & 0xC);
        }
        GX_FIFO_F32(-bx + cur_x);
        GX_FIFO_F32(-by + cur_y);
        GX_FIFO_F32(cur_z);
        if (pp->kind & DispTexture) {
            GX_FIFO_U8(((pp->kind >> 16) & 0xC) + 1);
        }
