...
/* SIS text streams are big-endian byte streams on disc and as built at
 * runtime; 16-bit tokens are read through these. */
#ifdef TARGET_PC
#include <string.h>
...
#ifdef TARGET_PC
        /* Pointer tokens are read as host words (relocated disc pointers
         * already are), so store this one in host order too. */
        memcpy(&text->string_buffer[text->x6C], &cursor, 4);
        text->x6C += 4;
#else
